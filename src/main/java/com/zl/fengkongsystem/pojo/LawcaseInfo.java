package com.zl.fengkongsystem.pojo;


public class LawcaseInfo {

  private String aimmaterials;
  private Double aimsum;
  private String applydate;
  private String approvedate;
  private Double baddebtsum;
  private String casebrief;
  private String casephase;
  private String casestatus;
  private String claim;
  private String cognizanceresult;
  private Double concessionsum;
  private String courtstatus;
  private String currency;
  private String declaredate;
  private String dunborrowerdate;
  private String dunguarantordate;
  private String effientwritbrief;
  private String endcasereason;
  private Double enforcedcost;
  private Double indebtinterest;
  private String inputdate;
  private String inputorgid;
  private String inputuserid;
  private String lawcasename;
  private String lawcaseno;
  private String lawcaseorg;
  private String lawcasetype;
  private String lawsuitstatus;
  private String lawsuittype;
  private Double legalcost;
  private Double loansum;
  private String manageorgid;
  private String manageuserid;
  private String oldcasephase;
  private String oldcasestatus;
  private String oldcognizeresult;
  private String oldcourtstatus;
  private String operateorgid;
  private String operateuserid;
  private Double othercost;
  private Double outdebtinterest;
  private String paystandard;
  private String pigeonholedate;
  private Double principal;
  private Double reclaimsum;
  private String remark;
  private String rightstype;
  private String saveornot;
  private String serialno;
  private String updatedate;
  private String lawcaseinfo;


  public String getAimmaterials() {
    return aimmaterials;
  }

  public void setAimmaterials(String aimmaterials) {
    this.aimmaterials = aimmaterials;
  }


  public Double getAimsum() {
    return aimsum;
  }

  public void setAimsum(Double aimsum) {
    this.aimsum = aimsum;
  }


  public String getApplydate() {
    return applydate;
  }

  public void setApplydate(String applydate) {
    this.applydate = applydate;
  }


  public String getApprovedate() {
    return approvedate;
  }

  public void setApprovedate(String approvedate) {
    this.approvedate = approvedate;
  }


  public Double getBaddebtsum() {
    return baddebtsum;
  }

  public void setBaddebtsum(Double baddebtsum) {
    this.baddebtsum = baddebtsum;
  }


  public String getCasebrief() {
    return casebrief;
  }

  public void setCasebrief(String casebrief) {
    this.casebrief = casebrief;
  }


  public String getCasephase() {
    return casephase;
  }

  public void setCasephase(String casephase) {
    this.casephase = casephase;
  }


  public String getCasestatus() {
    return casestatus;
  }

  public void setCasestatus(String casestatus) {
    this.casestatus = casestatus;
  }


  public String getClaim() {
    return claim;
  }

  public void setClaim(String claim) {
    this.claim = claim;
  }


  public String getCognizanceresult() {
    return cognizanceresult;
  }

  public void setCognizanceresult(String cognizanceresult) {
    this.cognizanceresult = cognizanceresult;
  }


  public Double getConcessionsum() {
    return concessionsum;
  }

  public void setConcessionsum(Double concessionsum) {
    this.concessionsum = concessionsum;
  }


  public String getCourtstatus() {
    return courtstatus;
  }

  public void setCourtstatus(String courtstatus) {
    this.courtstatus = courtstatus;
  }


  public String getCurrency() {
    return currency;
  }

  public void setCurrency(String currency) {
    this.currency = currency;
  }


  public String getDeclaredate() {
    return declaredate;
  }

  public void setDeclaredate(String declaredate) {
    this.declaredate = declaredate;
  }


  public String getDunborrowerdate() {
    return dunborrowerdate;
  }

  public void setDunborrowerdate(String dunborrowerdate) {
    this.dunborrowerdate = dunborrowerdate;
  }


  public String getDunguarantordate() {
    return dunguarantordate;
  }

  public void setDunguarantordate(String dunguarantordate) {
    this.dunguarantordate = dunguarantordate;
  }


  public String getEffientwritbrief() {
    return effientwritbrief;
  }

  public void setEffientwritbrief(String effientwritbrief) {
    this.effientwritbrief = effientwritbrief;
  }


  public String getEndcasereason() {
    return endcasereason;
  }

  public void setEndcasereason(String endcasereason) {
    this.endcasereason = endcasereason;
  }


  public Double getEnforcedcost() {
    return enforcedcost;
  }

  public void setEnforcedcost(Double enforcedcost) {
    this.enforcedcost = enforcedcost;
  }


  public Double getIndebtinterest() {
    return indebtinterest;
  }

  public void setIndebtinterest(Double indebtinterest) {
    this.indebtinterest = indebtinterest;
  }


  public String getInputdate() {
    return inputdate;
  }

  public void setInputdate(String inputdate) {
    this.inputdate = inputdate;
  }


  public String getInputorgid() {
    return inputorgid;
  }

  public void setInputorgid(String inputorgid) {
    this.inputorgid = inputorgid;
  }


  public String getInputuserid() {
    return inputuserid;
  }

  public void setInputuserid(String inputuserid) {
    this.inputuserid = inputuserid;
  }


  public String getLawcasename() {
    return lawcasename;
  }

  public void setLawcasename(String lawcasename) {
    this.lawcasename = lawcasename;
  }


  public String getLawcaseno() {
    return lawcaseno;
  }

  public void setLawcaseno(String lawcaseno) {
    this.lawcaseno = lawcaseno;
  }


  public String getLawcaseorg() {
    return lawcaseorg;
  }

  public void setLawcaseorg(String lawcaseorg) {
    this.lawcaseorg = lawcaseorg;
  }


  public String getLawcasetype() {
    return lawcasetype;
  }

  public void setLawcasetype(String lawcasetype) {
    this.lawcasetype = lawcasetype;
  }


  public String getLawsuitstatus() {
    return lawsuitstatus;
  }

  public void setLawsuitstatus(String lawsuitstatus) {
    this.lawsuitstatus = lawsuitstatus;
  }


  public String getLawsuittype() {
    return lawsuittype;
  }

  public void setLawsuittype(String lawsuittype) {
    this.lawsuittype = lawsuittype;
  }


  public Double getLegalcost() {
    return legalcost;
  }

  public void setLegalcost(Double legalcost) {
    this.legalcost = legalcost;
  }


  public Double getLoansum() {
    return loansum;
  }

  public void setLoansum(Double loansum) {
    this.loansum = loansum;
  }


  public String getManageorgid() {
    return manageorgid;
  }

  public void setManageorgid(String manageorgid) {
    this.manageorgid = manageorgid;
  }


  public String getManageuserid() {
    return manageuserid;
  }

  public void setManageuserid(String manageuserid) {
    this.manageuserid = manageuserid;
  }


  public String getOldcasephase() {
    return oldcasephase;
  }

  public void setOldcasephase(String oldcasephase) {
    this.oldcasephase = oldcasephase;
  }


  public String getOldcasestatus() {
    return oldcasestatus;
  }

  public void setOldcasestatus(String oldcasestatus) {
    this.oldcasestatus = oldcasestatus;
  }


  public String getOldcognizeresult() {
    return oldcognizeresult;
  }

  public void setOldcognizeresult(String oldcognizeresult) {
    this.oldcognizeresult = oldcognizeresult;
  }


  public String getOldcourtstatus() {
    return oldcourtstatus;
  }

  public void setOldcourtstatus(String oldcourtstatus) {
    this.oldcourtstatus = oldcourtstatus;
  }


  public String getOperateorgid() {
    return operateorgid;
  }

  public void setOperateorgid(String operateorgid) {
    this.operateorgid = operateorgid;
  }


  public String getOperateuserid() {
    return operateuserid;
  }

  public void setOperateuserid(String operateuserid) {
    this.operateuserid = operateuserid;
  }


  public Double getOthercost() {
    return othercost;
  }

  public void setOthercost(Double othercost) {
    this.othercost = othercost;
  }


  public Double getOutdebtinterest() {
    return outdebtinterest;
  }

  public void setOutdebtinterest(Double outdebtinterest) {
    this.outdebtinterest = outdebtinterest;
  }


  public String getPaystandard() {
    return paystandard;
  }

  public void setPaystandard(String paystandard) {
    this.paystandard = paystandard;
  }


  public String getPigeonholedate() {
    return pigeonholedate;
  }

  public void setPigeonholedate(String pigeonholedate) {
    this.pigeonholedate = pigeonholedate;
  }


  public Double getPrincipal() {
    return principal;
  }

  public void setPrincipal(Double principal) {
    this.principal = principal;
  }


  public Double getReclaimsum() {
    return reclaimsum;
  }

  public void setReclaimsum(Double reclaimsum) {
    this.reclaimsum = reclaimsum;
  }


  public String getRemark() {
    return remark;
  }

  public void setRemark(String remark) {
    this.remark = remark;
  }


  public String getRightstype() {
    return rightstype;
  }

  public void setRightstype(String rightstype) {
    this.rightstype = rightstype;
  }


  public String getSaveornot() {
    return saveornot;
  }

  public void setSaveornot(String saveornot) {
    this.saveornot = saveornot;
  }


  public String getSerialno() {
    return serialno;
  }

  public void setSerialno(String serialno) {
    this.serialno = serialno;
  }


  public String getUpdatedate() {
    return updatedate;
  }

  public void setUpdatedate(String updatedate) {
    this.updatedate = updatedate;
  }


  public String getLawcaseinfo() {
    return lawcaseinfo;
  }

  public void setLawcaseinfo(String lawcaseinfo) {
    this.lawcaseinfo = lawcaseinfo;
  }

}
